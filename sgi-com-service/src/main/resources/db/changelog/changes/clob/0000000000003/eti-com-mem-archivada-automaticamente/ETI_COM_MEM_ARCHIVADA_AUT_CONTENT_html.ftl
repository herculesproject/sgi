...
<html>
  ...
  <body>
    ...
    <p>ante la ausencia de respuesta a las modificaciones solicitadas por el/la <b>${data.nombreInvestigacion}</b>, con respecto a la solicitud de evaluación</p>
    <p>del/de la <b>${data.tipoActividad}</b> con título <b>${data.tituloSolicitudEvaluacion}</b> asociado/a a la memoria con referencia <b>${data.referenciaMemoria}</b>, le informamos que la situación de dicha solicitud pasará a archivada, debiendo enviar una nueva solicitud con el fin de obtener el correspondiente informe.</p>
