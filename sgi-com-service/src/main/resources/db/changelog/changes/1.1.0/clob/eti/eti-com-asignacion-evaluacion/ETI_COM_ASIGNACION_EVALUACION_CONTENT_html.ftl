...
<p>Us informem que en la propera convocatòria de reunió s'avaluarà la memòria referenciada:</p>
<p>
- Data de la reunió de la comissió: ${sgi.formatDate(data.fechaConvocatoriaReunion, "SHORT")}<br>
- Memòria a avaluar: ${data.referenciaMemoria}<br>
- Projecte: ${sgi.getFieldValue(data.tituloSolicitudEvaluacion)}
</p>
<p>
El parell avaluador assignat és el següent:<br>
${data.nombreApellidosEvaluador1}<br>
${data.nombreApellidosEvaluador2}
</p>
<#if data.fechaEvaluacionAnterior??>
<p>Aquesta memòria va obtenir un dictamen "pendent de correccions" en l'avaluació realitzada el ${sgi.formatDate(data.fechaEvaluacionAnterior, "SHORT")}. Podeu descarregar l'informe corresponent a l'aplicació: <a href="${data.enlaceAplicacion}" target="_blank"><b>${data.enlaceAplicacion}</b></a>.</p>
</#if>
<p>
