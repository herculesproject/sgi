...
Us informem que s'ha publicat la resolució provisional de la convocatòria sota referenciada en què la vostra sol·licitud apareix proposada per a finançament:
- Data de la resolució: <#if (data.fechaProvisionalConvocatoria)?has_content>${sgi.formatDate(data.fechaProvisionalConvocatoria, "SHORT")}<#else>-</#if>
