...
<p>Us informem que s'ha registrat una sol·licitud, indicant que vostè participarà com a tutor/a del treball associat.</p>
<p>
- Data de sol·licitud: ${sgi.formatDate(data.fechaEstado, "SHORT")}<br>
- Persona que registra la sol·licitud: ${data.nombreApellidosSolicitante}<br>
- Sol·licitud registrada: ${data.codigoInternoSolicitud}<br>
- Convocatòria: <#if data.tituloConvocatoria?has_content>${sgi.getFieldValue(data.tituloConvocatoria)}<#else>-</#if><br>
- Data de resolució provisional: <#if data.fechaProvisionalConvocatoria??>${sgi.formatDate(data.fechaProvisionalConvocatoria, "SHORT")}<#else>-</#if>
</p>
