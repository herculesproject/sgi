...
import { Direction, FilterType, FindOptions } from './types';
...
class FakeService extends BaseRestService<DummyData> {
...
describe('BaseRestService', () => {
  ...
  it('update() should PUT and return data', () => {
    ...
    const req = httpMock.expectOne(
      `${fakeEndpoint}/${existingEntity.id}`,
      'PUT to API'
    );
    // then: the right HTTP method is used to call the backend API
  ...
  it('update() non existing entity should throw error', () => {
    ...
    // then: the right backend API is called
    const req = httpMock.expectOne(
      `${fakeEndpoint}/${nonExistingEntity.id}`,
      'PUT to API'
    );
    // then: the right HTTP method is used to call the backend API
    expect(req.request.method).toBe('PUT');
  ...
  it('findAll(options.page) should GET ListResult paged with one element', () => {
    ...
    expect(req.request.headers.get('X-Page')).toBe(
      findOptions.page.index.toString()
    );
    // then: the right Pagination index header
    expect(req.request.headers.get('X-Page-Size')).toBe(
      findOptions.page.size.toString()
    );
  ...
  it('findAll(options.filters) with GREATHER, EQUALS, NOT_EQUALS and LOWER should GET ListResult with filter parameter', () => {
    ...
    // then: the right backend API is called
    const req = httpMock.expectOne(
      `${fakeEndpoint}?q=${encodeURI(
        'id>1,name:George,surname!:Watson,age<50'
      )}`,
      'GET with filter to API'
    );
    // then: the right HTTP method is used to call the backend API
    expect(req.request.method).toBe('GET');
  ...
  it(
    'findAll(options.filters) with GREATHER_OR_EQUAL, LIKE, NOT_LIKE and ' +
      'LOWER_OR_EQUAL should GET ListResult with filter parameter',
    () => {
      // given: find filters options
      const findOptions: FindOptions = {
        filters: [
  ...
  it('findAll(options) should GET ListResult paged with sort and filter parameters', () => {
    ...
    const req = httpMock.expectOne(
      ...
      'GET pagination with filter and sort to API'
    );
    // then: the right HTTP method is used to call the backend API
    expect(req.request.method).toBe('GET');
    // then: the right Pagination page header
