...
@Component({
  selector: 'sgi-peticion-evaluacion-tareas-crear-modal',
  templateUrl: './peticion-evaluacion-tareas-crear-modal.component.html',
  styleUrls: ['./peticion-evaluacion-tareas-crear-modal.component.scss']
})
export class PeticionEvaluacionTareasCrearModalComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public readonly matDialogRef: MatDialogRef<PeticionEvaluacionTareasCrearModalComponent>,
    ...
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'constructor()', 'start');
    this.fxLayoutProperties = new FxLayoutProperties();
    this.fxLayoutProperties.layout = 'column';
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'constructor()', 'end');
  }

  ngOnInit(): void {
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'ngOnInit()', 'start');
    ...
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'ngOnInit()', 'end');
  ...
  private initFormGroup() {
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'initFormGroup()', 'start');
    this.formGroup = new FormGroup({
      tarea: new FormControl('', [Validators.required]),
      organismo: new FormControl(''),
      anio: new FormControl(''),
      formacionEspecifica: new FormControl('', [Validators.required]),
      memoria: new FormControl('', [Validators.required]),
      equipoTrabajo: new FormControl('', [Validators.required])
    });
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'initFormGroup()', 'end');
  }
  ...
  closeModal(tarea?: ITarea): void {
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'closeModal()', 'start');
    this.matDialogRef.close(tarea);
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'closeModal()', 'end');
  }
   ...
   */
  addTarea() {
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'editTarea()', 'start');
    if (FormGroupUtil.valid(this.formGroup)) {
      this.closeModal(this.getDatosForm());
    } else {
      this.snackBarService.showError(MSG_ERROR_FORM);
    }
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'editTarea()', 'end');
  }
      ...
      this.personaServiceOneSubscritpion = this.personaFisicaService.getInformacionBasica(equipoTrabajo.personaRef)
        .subscribe(
          ...
          () => {
            ...
            this.logger.debug(
              PeticionEvaluacionTareasCrearModalComponent.name,
              'loadDatosUsuario()',
              'end'
            );
          }
  ...
  private getDatosForm(): ITarea {
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'getDatosForm()', 'start');
    const tarea: ITarea = {
      anio: FormGroupUtil.getValue(this.formGroup, 'anio'),
      equipoTrabajo: FormGroupUtil.getValue(this.formGroup, 'equipoTrabajo'),
      formacionEspecifica: FormGroupUtil.getValue(this.formGroup, 'formacionEspecifica'),
      formacion: null,
      id: null,
      memoria: FormGroupUtil.getValue(this.formGroup, 'memoria'),
      organismo: FormGroupUtil.getValue(this.formGroup, 'organismo'),
      tarea: FormGroupUtil.getValue(this.formGroup, 'tarea'),
      tipoTarea: null
    };
    ...
    // this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'getDatosForm()', 'end');
    return tarea;
  }

  ngOnDestroy(): void {
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'ngOnDestroy()', 'start');
    this.tareaSuscripcion?.unsubscribe();
    this.mostrarOrganismo$?.unsubscribe();
    this.logger.debug(PeticionEvaluacionTareasCrearModalComponent.name, 'ngOnDestroy()', 'end');
  }
