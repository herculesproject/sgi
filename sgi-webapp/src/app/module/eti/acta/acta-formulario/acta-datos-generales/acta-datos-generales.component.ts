...
export class ActaDatosGeneralesComponent extends FormFragmentComponent<IActa> implements OnInit {
  ...
  private setupI18N(): void {
    ...
    this.translate.get(
      ACTA_HORA_FIN_KEY,
      MSG_PARAMS.CARDINALIRY.SINGULAR
    ).subscribe((value) => this.msgParamHoraFinEntity = { entity: value, ...MSG_PARAMS.GENDER.FEMALE });
    ...
    this.translate.get(
      ACTA_MINUTO_FIN_KEY,
      MSG_PARAMS.CARDINALIRY.SINGULAR
    ).subscribe((value) => this.msgParamMinutoFinEntity = { entity: value, ...MSG_PARAMS.GENDER.MALE });
