...
export class ConvocatoriaConceptoGastoFragment extends FormFragment<IConvocatoriaConceptoGasto> {
  ...
  constructor(
    ...
    super(key);
    if (this.convocatoriaConceptoGasto) {
      this.convocatoriaConceptoGasto.convocatoria = this.convocatoria;
      this.convocatoriaConceptoGasto.permitido = this.permitido;
    } else {
  ...
  protected buildFormGroup(): FormGroup {
    const form = new FormGroup({
      conceptoGasto: new FormControl(this.convocatoriaConceptoGasto?.conceptoGasto, [IsEntityValidator.isValid()]),
      importeMaximo: new FormControl(this.convocatoriaConceptoGasto?.importeMaximo, [Validators.compose(
        [Validators.min(0), NumberValidator.maxDecimalPlaces(5)])]),
      mesInicial: new FormControl(this.convocatoriaConceptoGasto?.mesInicial, [Validators.compose(
        [Validators.min(1), this.convocatoria?.duracion ? Validators.max(this.convocatoria?.duracion) : Validators.max(9999), NumberValidator.isInteger()])]),
      mesFinal: new FormControl(this.convocatoriaConceptoGasto?.mesFinal, [Validators.compose(
        [Validators.min(1), this.convocatoria?.duracion ? Validators.max(this.convocatoria?.duracion) : Validators.max(9999), NumberValidator.isInteger()])]),
      observaciones: new FormControl(this.convocatoriaConceptoGasto?.observaciones)
    });
    if (this.readonly) {
      form.disable();
    }
 ...
 */
  private fieldsAreEqualOrSmallerThanNumber(firstNumberFieldName: string, secondNumberFieldName: string, numberCompare: number): ValidatorFn {
    return (formGroup: FormGroup): ValidationErrors | null => {
      ...
      if (numberCompare) {
        ...
        if ((numeroFirstControl.errors && !numeroFirstControl.errors.before) || (numeroSecondControl.errors && !numeroSecondControl.errors.before)) {
          return;
        }

        const numeroAnterior = (numeroSecondControl.value ? numeroSecondControl.value as number : 12) - (numeroFirstControl.value ? numeroFirstControl.value as number : 1);
  ...
  private notOverlapsSameConceptoGasto(startRangeFieldName: string, endRangeFieldName: string, filterFieldName: string): ValidatorFn {
    return (formGroup: FormGroup): ValidationErrors | null => {
      ...
      if (filterFieldControl.value) {
        ...
        const convocatoriaConceptoGastos = this.selectedConvocatoriaConceptoGastos.filter(convocatoriaConceptoGasto => convocatoriaConceptoGasto.id !== this.convocatoriaConceptoGasto.id).filter(convocatoriaConceptoGasto => convocatoriaConceptoGasto.conceptoGasto.id === filterFieldControl.value?.id);
