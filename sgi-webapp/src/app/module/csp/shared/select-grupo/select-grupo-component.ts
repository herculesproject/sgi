...
export class SelectGrupoComponent extends SelectServiceComponent<IGrupo> {
  ...
  private requestByPersonaRef = false;
  ...
  protected loadServiceOptions(): Observable<IGrupo[]> {
    ...
    if (this.requestByPersonaRef && this.personaRef) {
      // If empty, null or zero, an empty array is returned
      if (!!!this.personaRef) {
        return of([]);
      }
      findOptions.filter = new RSQLSgiRestFilter('solicitud.solicitanteRef', SgiRestFilterOperator.EQUALS, this.personaRef);
    }
