...
export interface IGrupoUnidadVinculacionListado {
  id: number;
  grupoId: number;
...
export class GrupoUnidadesVinculacionFragment extends Fragment {
  unidadesVinculacion$ = new BehaviorSubject<StatusWrapper<IGrupoUnidadVinculacionListado>[]>([]);
  ...
  protected onInitialize(): void {
    ...
    const grupoId = this.getKey() as number;

    this.subscriptions.push(
      this.grupoService.findUnidadesVinculacion(grupoId).pipe(
        ...
        this.unidadesVinculacion$.next(items.map(item => {
          ...
          return new StatusWrapper<IGrupoUnidadVinculacionListado>(item);
        }));
      })
    );
  }

  addGrupoUnidad(element: IGrupoUnidadVinculacionListado): void {
    const wrapper = new StatusWrapper<IGrupoUnidadVinculacionListado>(element);
    wrapper.setCreated();
  ...
  deleteGrupoUnidad(wrapper: StatusWrapper<IGrupoUnidadVinculacionListado>): void {
    const current = this.unidadesVinculacion$.value;
  ...
  saveOrUpdate(): Observable<void> {
    ...
    return this.grupoService.updateUnidadesVinculacion(grupoId, unidadesVinculacion).pipe(
      tap(() => {
        this.unidadesVinculacion$.next(
          this.unidadesVinculacion$.value.map(wrapper => new StatusWrapper<IGrupoUnidadVinculacionListado>(wrapper.value))
        );
