...
export class TipoProteccionDatosGeneralesFragment extends FormFragment<ITipoProteccion> {
  ...
  getValue(): ITipoProteccion {
    ...
    tipoProteccion.tipoPropiedad = form.tipoPropiedad;
    return tipoProteccion;
  }

  saveOrUpdate(): Observable<number> {
    const tiposProteccion = this.getValue();
    const observable$ = this.isEdit() ? this.update(tiposProteccion) : this.create(tiposProteccion);
