      ...
      (modalData: EmpresaEquipoEmprendedorModalData) => {
        if (modalData) {
          if (!wrapper) {
            ...
            const entidad = new StatusWrapper<IEmpresaEquipoEmprendedorListado>(modalData.entidad as IEmpresaEquipoEmprendedorListado);
            this.formPart.updateEmpresaEquipoEmprendedor(entidad);
          }
        }
      }
    );
