-- TIPO_ENLACE
insert into test.tipo_enlace (id,activo) values (1,true);
insert into test.tipo_enlace (id,activo) values (2,true);
insert into test.tipo_enlace (id,activo) values (3,false);
insert into test.tipo_enlace (id,activo) values (4,true);
insert into test.tipo_enlace (id,activo) values (5,true);
insert into test.tipo_enlace (id,activo) values (6,false);

-- TIPO_ENLACE_NOMBRE
INSERT INTO test.tipo_enlace_nombre (tipo_enlace_id, lang, value_) VALUES(1, 'es', 'nombre-001');
INSERT INTO test.tipo_enlace_nombre (tipo_enlace_id, lang, value_) VALUES(2, 'es', 'nombre-002');
INSERT INTO test.tipo_enlace_nombre (tipo_enlace_id, lang, value_) VALUES(3, 'es', 'nombre-003');
INSERT INTO test.tipo_enlace_nombre (tipo_enlace_id, lang, value_) VALUES(4, 'es', 'nombre-011');
INSERT INTO test.tipo_enlace_nombre (tipo_enlace_id, lang, value_) VALUES(5, 'es', 'nombre-012');
INSERT INTO test.tipo_enlace_nombre (tipo_enlace_id, lang, value_) VALUES(6, 'es', 'nombre-013');

-- TIPO_ENLACE_DESCIPCION
INSERT INTO test.tipo_enlace_descripcion (tipo_enlace_id, lang, value_) VALUES(1, 'es', 'descripcion-001');
INSERT INTO test.tipo_enlace_descripcion (tipo_enlace_id, lang, value_) VALUES(2, 'es', 'descripcion-002');
INSERT INTO test.tipo_enlace_descripcion (tipo_enlace_id, lang, value_) VALUES(3, 'es', 'descripcion-003');
INSERT INTO test.tipo_enlace_descripcion (tipo_enlace_id, lang, value_) VALUES(4, 'es', 'descripcion-011');
INSERT INTO test.tipo_enlace_descripcion (tipo_enlace_id, lang, value_) VALUES(5, 'es', 'descripcion-012');
INSERT INTO test.tipo_enlace_descripcion (tipo_enlace_id, lang, value_) VALUES(6, 'es', 'descripcion-013');