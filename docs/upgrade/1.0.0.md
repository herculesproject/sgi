...
>
> [Resumen de cambios (CHANGELOG)](../../CHANGELOG.md#100-2026-02-23) | [Guía de despliegue del SGI con Helm](common/deployment.md)
...
Los valores definidos a nivel de servicio (`service-name.image.tag`) tienen prioridad sobre la configuración global. Para que el versionado centralizado sea efectivo, es necesario eliminar las definiciones específicas de `tag` y `pullPolicy` en cada servicio; de lo contrario, se mantendrán los valores existentes por compatibilidad.

#### Compatibilidad con gestión individual
El método de actualización individual por servicio sigue siendo totalmente funcional. Se puede optar por no usar la clave global `sgi` y continuar especificando la versión deseada en cada componente de forma independiente.

#### Servicios de integración
Mantener la configuración específica para los servicios de integración. Estos componentes suelen seguir un ciclo de vida de versiones distinto al núcleo del SGI, por lo que se recomienda conservar sus definiciones de `tag` y `pullPolicy` para asegurar un despliegue correcto.
...
4. Validación funcional:
    ...
    Confirmar el correcto funcionamiento de los servicios afectados por la actualización, prestando especial atención a los cambios incluidos en el [CHANGELOG](../../CHANGELOG.md#100-2026-02-23) de esta versión.
